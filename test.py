                ...
                with tab_resumo:
                    ...
                    with m1:
                        ...
                        if anotacoes_uniprot.get("signal_peptide"):
                            ...
                            st.info("Nenhum peptídeo sinal detectado nesta proteína.")

                        st.markdown("**🏛️ Domínios Funcionais & Regiões:**")
                        if anotacoes_uniprot.get("domains"):
                            for item in anotacoes_uniprot["domains"]:
                                st.write(f"• {item}")
                        else:
                            st.info("Nenhum domínio específico mapeado via API UniProt.")

                    with m2:
                        st.markdown("**🎯 Sítios Ativos & de Ligação:**")
                        if anotacoes_uniprot.get("binding_sites"):
                            for item in anotacoes_uniprot["binding_sites"]:
                                st.write(f"• {item}")
                        else:
                            st.info("Nenhum sítio de ligação/ativo mapeado via API UniProt.")

                        st.markdown("**🔗 Interações Proteína-Proteína (PPI):**")
                        if anotacoes_uniprot.get("interactions"):
                            for item in set(anotacoes_uniprot["interactions"]):
                                st.write(f"• {item}")
                        else:
                            st.info("Nenhuma interação direta catalogada via API UniProt.")

                with tab_3d:
                    st.markdown("##### 🧊 Estrutura Tridimensional & Análise Espacial")
                    pdb_data = obter_pdb_alphafold(uniprot_val)

                    if pdb_data:
                        col_viewer, col_analysis = st.columns([1.3, 1])

                        with col_viewer:
                            st.markdown("**Visualizador 3D (AlphaFold Structure)**")
                            if HAS_3DMOL:
                                view = py3Dmol.view(width=550, height=450)
                                view.addModel(pdb_data, "pdb")
                                view.setStyle({'cartoon': {'color': 'spectrum'}})

                                # Destacar Peptídeo Sinal se existente
                                sig_range = anotacoes_uniprot.get("signal_range")
                                if sig_range:
                                    view.addStyle({'resi': f"{sig_range[0]}-{sig_range[1]}"}, {'cartoon': {'color': 'red'}})

                                view.zoomTo()
                                showmol(view, height=450, width=550)
                            else:
                                st.warning("Instale a biblioteca `stmol` e `py3Dmol` para habilitar o visualizador 3D interativo na página.")
                                st.info("Comando: `pip install stmol py3Dmol`")

                        with col_analysis:
                            st.markdown("**Análise Espacial de Contatos (Rede 3D)**")
                            df_ca = extrair_coordenadas_ca(pdb_data)
                            
                            if not df_ca.empty:
                                raio_corte = st.slider("Distância de corte para contato 3D (Å):", 4.0, 12.0, 8.0, 0.5)
                                G, total_edges, df_ligacoes = calcular_rede_ligacoes(df_ca, distancia_corte=raio_corte)

                                st.metric("Total de Resíduos (C-Alpha)", f"{len(df_ca)}")
                                st.metric("Contatos Espaciais Totais", f"{total_edges:,}".replace(",", "."))

                                if not df_ligacoes.empty:
                                    st.markdown("**Top Resíduos Mais Conectados (Hubs):**")
                                    st.dataframe(
                                        df_ligacoes.head(10),
                                        use_container_width=True,
                                        hide_index=True
                                ...
                                st.error("Não foi possível extrair coordenadas C-Alpha do arquivo PDB.")
                    else:
                        st.warning(f"Não foi possível carregar o modelo 3D PDB para o ID UniProt `{uniprot_val}`.")
        else:
            st.warning(f"Nenhum registro encontrado para o gene **{gene_selecionado}**.")
